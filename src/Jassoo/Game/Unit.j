public struct Unit extends IWidget {
	...
	static method create (GamePlayer plr, integer typeId, Point pos, real face)->thistype {
		return thistype.Get(CreateUnit(Player(plr), typeId, pos.X, pos.Y, face));
	}
    ...
    static method Get (unit h)->thistype {
	...
	method DamageTarget (IWidget target, real damage, string damageType)->boolean {
		return UnitDamageTarget(this.h, Utils.GetWidget(target), damage, damageType == "Attack",
			false, ATTACK_TYPE_MELEE, thistype.getDamageType(damageType), WEAPON_TYPE_WHOKNOWS);
	}
	private static method getDamageType (string damageType)->damagetype {
        ...
        } else if (damageType == "Universal") {
            return DAMAGE_TYPE_UNIVERSAL;
        }
        debug Log.Error("thistype.getDamageType", "unknow damage type(" + damageType + ")");
		return DAMAGE_TYPE_UNIVERSAL;
	}
	...
	method OrderTarget (integer orderId, IWidget target)->boolean {
		return IssueTargetOrderById(this.h, orderId, Utils.GetWidget(target)); }
	...
	private static method onInit () {
        ...
        GroupEnumUnitsInRect(g, Region.WorldRect, Condition(function ()->boolean {
	        ...
	        Group.WorldGroup.Add(thistype.Get(u));
		...
		Event.EnterRegion.AddAction(function (integer i) {
			if (Event.Region == Region.WorldRegion) {
				Group.WorldGroup.Add(thistype.Get(GetEnteringUnit()));
			}
		});
        
		Event.LeaveRegion.AddAction(function (thistype u) {
			if (Event.Region== Region.WorldRegion) {
				Group.WorldGroup.Remove(u);
				Utils.FlushInteger(u.HandleId);
				u.h = null;
				u.Point.destroy();
			}
		});
