// no confliction!
private {
    constant integer player_MaxCount = 16;
    function clearSelection () { ClearSelection(); }
    function selectUnit (unit u, boolean flag) { SelectUnit(u, flag); }
}

public struct GamePlayer [player_MaxCount] {
	private Group selectedUnits;
	private player h;
    
    static method operator LocalPlayer ()->thistype { return GetPlayerId(GetLocalPlayer()); }
